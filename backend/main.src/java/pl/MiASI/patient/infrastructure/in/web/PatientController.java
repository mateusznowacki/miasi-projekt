...
class PatientController {
    ...
    public ResponseEntity<Void> addMedicalRecord(@PathVariable UUID patientId, @Valid @RequestBody AddMedicalRecordReq req) {
        try {
            patientUseCase.addMedicalRecord(new PatientId(patientId), req.visitId(), new pl.MiASI.shared.application.domain.model.DoctorId(req.doctorId()), req.diagnoses(), req.symptoms(), req.prescriptions(), req.notes(), req.testResults());
    ...
    public ResponseEntity<Void> updateMedicalRecord(@PathVariable UUID patientId, @PathVariable UUID recordId, @Valid @RequestBody UpdateMedicalRecordReq req, org.springframework.security.core.Authentication authentication) {
        try {
            pl.MiASI.shared.application.domain.model.DoctorId doctorId = new pl.MiASI.shared.application.domain.model.DoctorId(UUID.fromString(authentication.getName()));
    ...
    public ResponseEntity<pl.MiASI.patient.application.domain.model.MedicalRecord> getMedicalRecordByVisitId(@PathVariable UUID patientId, @PathVariable UUID visitId) {
    ...
    public ResponseEntity<pl.MiASI.patient.application.domain.model.Patient> getPatientProfile(@PathVariable UUID patientId) {
    ...
    public ResponseEntity<java.util.List<pl.MiASI.patient.application.domain.model.MedicalRecord>> getMedicalHistory(@PathVariable UUID patientId) {
        return patientUseCase.getPatientProfile(new PatientId(patientId))
    ...
    @GetMapping("/search")
    public ResponseEntity<java.util.List<pl.MiASI.patient.application.domain.model.Patient>> searchPatients(
